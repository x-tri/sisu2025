...
def sync_course_students(course_id: int, code: int):
    ...
    try:
        ...
        requests.delete(f"{SUPABASE_URL}/rest/v1/approved_students?course_id=eq.{course_id}", headers=HEADERS, timeout=30)
...
def main():
    ...
    resp = requests.get(f"{SUPABASE_URL}/rest/v1/approved_students?select=course_id&limit=1", headers=HEADERS, timeout=30)
    has_students = False 
