...
def sync_courses_by_params(name, params):
    print(f"Fetching {name} courses from Supabase...")
    resp = requests.get(f"{SUPABASE_URL}/rest/v1/courses", headers=HEADERS, params=params)
    courses = resp.json()
    print(f"Found {len(courses)} {name} courses. Syncing...")
    ...
    for i, course in enumerate(courses):
        ...
        try:
            ...
            print(f"[{i+1}/{len(courses)}] {course['name']}: {added_for_course} records")
            total_added += added_for_course
            
        except Exception as e:
            print(f"Error {code}: {e}")
            
    print(f"{name}: Total Added: {total_added}")

if __name__ == "__main__":
    # UFERSA
    sync_courses_by_params("UFERSA", {
        "select": "id,code,name",
        "university": "ilike.*Universidade Federal Rural do Semi-Árido*"
    })
    
    # UFRN Caicó
    sync_courses_by_params("UFRN Caicó", {
        "select": "id,code,name",
        "university": "ilike.*Rio Grande do Norte*",
        "campus": "ilike.*Caic*"
    })
